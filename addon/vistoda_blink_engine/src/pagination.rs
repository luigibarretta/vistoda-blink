...
) -> Result<(Vec<T>, Pagination), EngineError> {
...
mod tests {
    ...
    #[test]
    fn returns_bounded_page_metadata() {
        let values: Vec<_> = (0..73).collect();
        let (items, result) =
            page(&values, Some(4), Some(20)).unwrap_or_else(|error| panic!("{error}"));
    ...
    fn rejects_unbounded_page_sizes() {
        assert!(page::<u8>(&[], Some(1), Some(101)).is_err());
        let (items, result) =
            page(&[0; 120], Some(1), Some(100)).unwrap_or_else(|error| panic!("{error}"));
